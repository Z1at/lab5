...
public class FieldReceiverForFile {
    public static String getName(FileReader file) throws IOException {
        while(file.ready()){
            String[] name = getLine(file).toLowerCase().strip().split(" ");
    ...
    public static Coordinates getCoordinates(FileReader file) throws IOException {
        while(file.ready()){
            String[] coordinates = getLine(file).toLowerCase().strip().split(" ");
    ...
    public static double getArea(FileReader file) throws IOException {
        while(file.ready()){
            String[] area = getLine(file).toLowerCase().strip().split(" ");
            if(area.length == 1){
                try{
    ...
    public static Long getPopulation(FileReader file) throws IOException {
        while(file.ready()){
            String[] population = getLine(file).toLowerCase().strip().split(" ");
    ...
    public static Integer getMetersAboveSeaLevel(FileReader file) throws IOException {
        while(file.ready()){
            String[] metersAboveSeaLevel = getLine(file).toLowerCase().strip().split(" ");
    ...
    public static Climate getClimate(FileReader file) throws IOException {
        while(file.ready()){
            String[] climate = getLine(file).toUpperCase().strip().split(" ");
    ...
    public static Government getGovernment(FileReader file) throws IOException {
        while(file.ready()){
            String [] government = getLine(file).toUpperCase().strip().split(" ");
    ...
    public static StandardOfLiving getStandardOfLiving(FileReader file) throws IOException {
        while(file.ready()){
            String[] standardOfLiving = getLine(file).toUpperCase().strip().split(" ");
    ...
    public static Human getGovernor(FileReader file) throws IOException{
        String name = "";
        while(file.ready()){
            String[] array = getLine(file).toLowerCase().strip().split(" ");
        ...
        while(file.ready()){
            String[] height = getLine(file).strip().split(" ");
            if(height.length == 1){
                try{
